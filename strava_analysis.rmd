...
```{r include=FALSE}
strava$moving_time <- round(strava$moving_time/60,2)
strava$start_date_local <- as.Date(strava$start_date_local)
strava$distance <- round(strava$distance/1000,2)
strava$average_speed <- round(strava$average_speed * 3.6,2)
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
first <- tail(strava,1)
knitr::kable(first[,c("start_date_local","name","moving_time","distance","average_speed","sport_type")])
```


```{r echo=FALSE, message=FALSE, warning=FALSE}
last <- head(strava,1)
knitr::kable(last[,c("start_date_local","name","moving_time","distance","average_speed","sport_type")])
```
...
strava$date <- strava$start_date_local
...
colors <- sample(col_vector, size = length(unique(strava$sport_type)))
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
strava$minutes <- as.numeric(strava$moving_time)
strava$km <- as.numeric(strava$distance)
...
strava$`km.h` <- as.numeric(strava$average_speed)  # use backticks for column with dot
...
# First check if ride and/or run are present
if(nrow(df_speed) != 0){
  df_speed <- df_speed %>%
    mutate(year = format(date, "%Y")) %>%
  ...
  message("No rides or runs were found in your activities.")
}
...
#add check if HR data is present
if(is.null(strava$average_heartrate)){
  message("No heartrate data could be found in your activities.")
} else{
  strava$HR_av <- as.numeric(strava$average_heartrate)
...
# Check if suffer_score column is present for non-premium users
# add column with NA suffer score for all activities if it is the case
if(is.null(strava$suffer_score)){
  strava$suffer_score <- rep(NA,nrow(strava))
}

if(is.null(strava$HR_av)){
  strava$HR_av <- rep(NA,nrow(strava))
}
...
extremes <- tibble(
  ...
  value = c(
    ...
    max(df$total_elevation_gain, na.rm = TRUE),
  ...
  name = c(
    ...
    df$name[which.max(df$total_elevation_gain)],
   ...
   sport = c(
    ...
    df$type[which.max(df$total_elevation_gain)],
...
library(viridis)
gear_props <- strava %>%
  filter(sport_type %in% c("Ride", "GravelRide", "MountainBikeRide","EBikeRide")) %>%
  mutate(year = format(date, "%Y")) %>%
  count(year, gear_name) %>%
  group_by(year) %>%
  mutate(prop = n / sum(n)) %>%
  ungroup()

gear_props$gear_name[which("" == (gear_props$gear_name))] <- "Unknown"
...
if(length(unique(gear_props$gear_name)) > 12){
  bike_colors <-viridis_pal(option = "D")(length(unique(gear_props$gear)))
}

  
names(bike_colors) <- unique(gear_props$gear_name)

# Create 100% stacked bar plot
ggplot(gear_props, aes(x = year, y = prop, fill = gear_name)) +
  geom_bar(stat = "identity", position = "fill") +
  scale_y_continuous(labels = scales::percent_format()) +
...
df_coords$lat <- as.numeric(df_coords$lat)
df_coords$lon <- as.numeric(df_coords$lon)
